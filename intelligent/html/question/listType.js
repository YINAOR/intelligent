(function () {
    ...
    var resultnull = { list: [] };
    _g.render('question/listType-V', resultnull, '#table');
                    ...
                    if(result.data.paging) {
                        ...
                        if(categoryList.list.length>0){
                            _g.initPaginator({
                             ...
                             _g.render('question/listType-V', categoryList, '#table');
                        } 
                    } else {
                        var resultnull = { list: [] };
                        _g.render('question/listType-V', resultnull, '#table');
    ...
    downloadExcelTemplet = function() {
        var token = sessionStorage.getItem('token');;
        window.location.href='http://120.77.204.252:80/category/downExcelTemplets.do?token='+token + '&parentCode=' + parentCode;
    }

    exportExcel = function() {
        var token = sessionStorage.getItem('token');
        window.location.href='http://120.77.204.252:80/category/exportExcel.do?token='+token + '&parentCode=' + parentCode;
    }
